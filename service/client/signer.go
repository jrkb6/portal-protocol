...
func NewReader(config Config) *TxReader {
	client, err := ethclient.Dial(config.Rawurl)
	if err != nil {
		log.Fatal("Couldn't connect to eth client", err)
	}
	return &TxReader{
		Client: client,
	}
...
func (s *Signer) Verify(digestHash []byte, signature []byte) bool {
	return crypto.VerifySignature(s.PublicKey, digestHash, signature[:64])
}
