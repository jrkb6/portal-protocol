...
import (
	...
	merkleCombined "go-sdk/contracts/MerkleCombined"
	mimcCombined "go-sdk/contracts/MimcCombined"
	"html/template"
	"io"
	"log"
	"math/big"
...
func CreateCircuitBindings(path string, circuitName string) {
	dir, err := os.Getwd()

	// compile solidity contract
	filePath := fmt.Sprintf("%s/%s.sol", path, circuitName)
	fullPath := filepath.Join(dir, filePath)
	...
	out := fmt.Sprintf("%s/%s/%s.%s", "--out=contracts", circuitName, circuitName, "go")
...
func CircuitToSolidity(vk plonk.VerifyingKey, solidityPath string) {
	...
	err = CustomExportSolidity(&vk, f)
	if err != nil {
		log.Fatal("vk.ExportSolidity ", err)
	}
}

func DeployMimcCircuit(signer *client.Signer) (common.Address, *types.Transaction) {

	auth := signer.BindTxOpts()
	circuitContractAddr, tx, _, err := mimcCombined.DeployMimcCombined(auth, signer.Client)
	if err != nil {
		log.Fatal(err)
	}
...
func DeployMerkleCircuit(signer *client.Signer) (common.Address, *types.Transaction) {

	auth := signer.BindTxOpts()
	circuitContractAddr, tx, _, err := merkleCombined.DeployMerkleCombined(auth, signer.Client)
	if err != nil {
		log.Fatal(err)
	}
...
func CustomExportSolidity(vk *plonk.VerifyingKey, w io.Writer) error {
